...
Сервер получает задачу, присваивает ей уникальный идентификатор, сохраняет информацию о задаче в памяти и запускает её обработку в отдельном потоке. Во время выполнения задача имеет статус processing. После завершения обработки статус изменяется на completed.
